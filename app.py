...
# =========================
@app.route("/admin/update-name/<int:user_id>/<string:novo_nome>")
def admin_update_name(user_id, novo_nome):
    conn = get_db_connection()
    user = conn.execute("SELECT is_admin FROM users WHERE id = ?", (session.get("user_id"),)).fetchone()
    if not user or user["is_admin"] != 1:
        conn.close()
        return jsonify({"status": "error", "message": "Acesso negado"}), 403
    conn.execute("UPDATE users SET name = ? WHERE id = ?", (novo_nome, user_id))
    conn.commit()
    conn.close()
    return jsonify({"status": "ok", "message": f"Nome actualizado para {novo_nome}"})
