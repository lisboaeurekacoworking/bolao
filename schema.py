...
def get_ranking_data(view=None, filter_value=None):
    ...
    base_query = """
        SELECT id, name, country_code, eureka_unit
        FROM users
    """
    ...
    if view == "country" and filter_value:
        base_query += " WHERE country_code = ?"
        params.append(filter_value)

    elif view == "unit" and filter_value:
        base_query += " WHERE eureka_unit = ?"
        params.append(filter_value)
    ...
    for index, user in enumerate(ranking_data, start=1):
        user["position"] = index
